...
function M2({diabetes, hbp, hbc}) {
    ...
    useEffect(() => {
      ...
      fetchmbData();
      setFdata(mbdata.filter(item => item &&  item.Type_M.includes("Two"))
    ...
    const getorTrans = (item) => {
      data.map(x => x.Food_Name.includes(item) ? (parseInt(x.Diabetes_Score) > 5 ? item : x.Similar_Choices): null);
    }
    return (
        <div className='M1d'>
          {mbdata.filter(item => item && item.Type_M.includes("Two")).map((row, index) => (
            <tr key={index}>
